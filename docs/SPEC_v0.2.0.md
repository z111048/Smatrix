...
**驗收標準**：
- [ ] 所有文字符合 WCAG 2.1 AA 標準（對比度 ≥4.5:1）
- [ ] 重要資訊符合 AAA 標準（對比度 ≥7:1）
- [ ] 按鈕有明確的 hover/focus 狀態
- [ ] 輸入框有可見的 focus 邊框
- [ ] 支援鍵盤導航（Tab 鍵順序合理）
...
|----------|------|------|
| 節點載重 | Fx, Fy, Mz | 三分量 |
| 垂直於元素 UDL | w_perp | 傳統梁載 |
| 平行於元素 UDL | w_para | 斜桿自重 |
| 任意角度 UDL | w, angle | 風壓 |
| 梯形分布載重 | w1, w2 | 變化強度 |
| 元素集中載重 | P, a | 任意位置 |
...
| T1-6 | 兩端固定梁 | 均佈載重 | M = wL²/12 |
| T1-7 | 斜桿 | 軸向載重 | δ = PL/EA |
| T1-8 | 斜桿 45° | 垂直載重 | 分解驗證 |
